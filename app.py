    ...
    if request.method == "POST":
        ...
        if reservation_time in tables_data_multiple_days:
            ...
            default_data = [
                ...
                {"table": 11, "state": True, "number_seats": 5, "number_seats_available": random.randint(0,5)},
                {"table": 12, "state": True, "number_seats": 5, "number_seats_available": random.randint(0,5)},
...
def users():
     ...
     return render_template('client_page.html', client_details=client_details)
    ...
    for reservation in reservations:
        for client in reservation['clients']:
            if client['id'] == client_id:
                ...
                client_found = True
                # Update the number_clients field
                reservation['number_clients'] = len(reservation['clients'])
                break
        if client_found:
            break
...
def check_all_status():
    current_time = datetime.now().time()
    opening_time = time(9, 0)  # Assuming opening time is 09:00 AM
    closing_time = time(17, 0)  # Assuming closing time is 05:00 PM
    ...
    for reservation in reservations:
        for client in reservation["clients"]:
            ...
            status_list.append(client_info)
...
def search_clients():
        ...
        for reservation in reservations:
            for client in reservation["clients"]:
                if client["name"] == search_name:
                    ...
                    search_results.append(client_info)
    elif search_reservations:
        search_reservations = int(search_reservations)
        for reservation in reservations:
            if reservation["table"] == search_reservations:
                for client in reservation["clients"]:
                    client_info = {
                        "table_number": reservation["table"],
                        "reservation_time": reservation["reservation_time"],
    ...
    return render_template('search_clients.html', search_results=search_results, search_name=search_name, search_reservations=search_reservations, search_phone=search_phone)
